...
<head>
    ...
    <title>@yield('title') - SIDESA Portal Warga</title>
    ...
    <!-- Bootstrap (For Modals/JS if needed, scoped to avoid conflicts) -->
...
<body class="antialiased overflow-x-hidden flex h-screen selection:bg-sidesa-500 selection:text-white">
    ...
    <aside id="sidebar" class="w-72 h-full flex flex-col fixed left-0 top-0 z-50 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-xl bg-gradient-to-b from-sidesa-900 to-sidesa-800">
        ...
        <div class="p-6 border-b border-white/10 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sidesa-100/80 text-xs font-medium m-0">Portal Warga</p>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
            ...
            <a href="{{ route('user.dashboard') }}" class="sidebar-item flex items-center gap-4 px-4 py-3 rounded-xl text-white font-medium {{ $currentRoute == 'user.dashboard' ? 'active' : '' }}">
                <div class="w-8 flex justify-center"><i class="fa-solid fa-house-chimney text-lg {{ $currentRoute == 'user.dashboard' ? 'text-sidesa-300' : 'text-sidesa-100/70' }}"></i></div>
                <span>Dashboard</span>
            </a>

            <a href="{{ route('profile') }}" class="sidebar-item flex items-center gap-4 px-4 py-3 rounded-xl text-white font-medium {{ Str::startsWith($currentRoute, 'profile') ? 'active' : '' }}">
                <div class="w-8 flex justify-center"><i class="fa-solid fa-user text-lg {{ Str::startsWith($currentRoute, 'profile') ? 'text-sidesa-300' : 'text-sidesa-100/70' }}"></i></div>
                <span>Profil Saya</span>
            </a>

            <a href="{{ route('pengaduan.index') }}" class="sidebar-item flex items-center gap-4 px-4 py-3 rounded-xl text-white font-medium {{ Str::startsWith($currentRoute, 'pengaduan') ? 'active' : '' }}">
                <div class="w-8 flex justify-center"><i class="fa-solid fa-comments text-lg {{ Str::startsWith($currentRoute, 'pengaduan') ? 'text-sidesa-300' : 'text-sidesa-100/70' }}"></i></div>
                <span>Pengaduan Saya</span>
            </a>

            <a href="{{ route('user.pengajuan.index') }}" class="sidebar-item flex items-center gap-4 px-4 py-3 rounded-xl text-white font-medium {{ Str::startsWith($currentRoute, 'user.pengajuan') ? 'active' : '' }}">
                <div class="w-8 flex justify-center"><i class="fa-solid fa-file-lines text-lg {{ Str::startsWith($currentRoute, 'user.pengajuan') ? 'text-sidesa-300' : 'text-sidesa-100/70' }}"></i></div>
                <span>Riwayat Pengajuan</span>
            </a>

            <a href="{{ route('kegiatan') }}" class="sidebar-item flex items-center gap-4 px-4 py-3 rounded-xl text-white font-medium {{ Str::startsWith($currentRoute, 'kegiatan') ? 'active' : '' }}">
                <div class="w-8 flex justify-center"><i class="fa-solid fa-calendar-days text-lg {{ Str::startsWith($currentRoute, 'kegiatan') ? 'text-sidesa-300' : 'text-sidesa-100/70' }}"></i></div>
                <span>Agenda Desa</span>
            </a>

            <a href="{{ route('berita.warta') }}" class="sidebar-item flex items-center gap-4 px-4 py-3 rounded-xl text-white font-medium {{ Str::startsWith($currentRoute, 'berita') ? 'active' : '' }}">
                <div class="w-8 flex justify-center"><i class="fa-solid fa-newspaper text-lg {{ Str::startsWith($currentRoute, 'berita') ? 'text-sidesa-300' : 'text-sidesa-100/70' }}"></i></div>
    ...
    <main class="flex-1 flex flex-col h-full lg:ml-72 w-full transition-all duration-300 relative z-10">
        ...
        <!-- Header -->
        <header class="glass-header sticky top-0 z-30 px-6 py-4 flex justify-between items-center h-20 shadow-sm">
            ...
            <a href="{{ route('profile') }}" class="flex items-center gap-4 cursor-pointer hover:bg-slate-50 p-2 rounded-xl transition-colors border border-transparent hover:border-slate-200">
                <div class="hidden md:flex flex-col items-end mr-2">
                    <span class="text-sm font-bold text-slate-700">{{ Auth::user()->name }}</span>
                    <span class="text-xs text-sidesa-600 font-semibold px-2 py-0.5 bg-sidesa-50 rounded-full border border-sidesa-100 mt-1">Warga Desa</span>
                </div>
